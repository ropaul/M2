turtles-own
  [ sick?                ;; if true, the turtle is infectious
    fascist?             ;; if true, kill sick people
    doctor?             ;; if true , heal people
...
to setup-turtles
  ...
  ask n-of 10 turtles
    [ get-sick ]
    ask n-of number-fascist turtles
    [ set fascist? true ]
    ask n-of number-doctor turtles
    [ set doctor? true ]
end
...
to update-display
  ask turtles
    [ if shape != turtle-shape [ set shape turtle-shape ]
      set color ifelse-value sick? [ red ] [ ifelse-value immune? [ grey ] [ifelse-value fascist? [ brown ] [ifelse-value doctor? [ yellow ] [ green ] ] ]]]
end
...
to kill-sick-people  ;; if fascist , kill sick people
  if fascist? = true
  [
  let prey with [sick?] one-of turtles-here                    ;; grab a random sheep
...
to heal-sick-people  ;; if fascist , kill sick people
  if doctor? = true
  [
  let prey with [sick?] one-of turtles-here                    ;; grab a random sheep
  if prey != nobody                              ;; did we get one?  if so,
    [ ask prey [ become-immune ]                          ;; kill it
    ]
  ]   
end
...
to reproduce
  ...
  [
    ...
    [
    ifelse count turtles with [fascist?] < number-fascist and random-float 100 < number-fascist
    [ hatch 1
      [ set age 1
        lt 45 fd 1
        get-healthy ] 
      ...
      set doctor? false]
    [
     ifelse count turtles with [doctor?] < number-doctor and random-float 100 < number-doctor
    [ hatch 1
      [ set age 1
        lt 45 fd 1
        get-healthy ] 
...
"fascist" 1.0 0 -955883 true "" "plot count turtles with [fascist?]"
"doctor" 1.0 0 -1184463 true "" "plot count turtles with [doctor?]"
