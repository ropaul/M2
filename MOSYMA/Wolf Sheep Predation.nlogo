...
to setup
  ...
  set-default-shape bears "cow"
  create-bears initial-number-bears  ;; create the wolves, then initialize their variables
  [
    set color red
    set size 4  ;; easier to see
    set energy random (2 * bears-gain-from-food)
    setxy random-xcor random-ycor
  ]
  
  
   set-default-shape hunters "person"
  create-hunters initial-number-hunters  ;; create the wolves, then initialize their variables
  [
    set color orange
    set size 2  ;; easier to see
    set energy random (hunters-gain-from-food)
    setxy random-xcor random-ycor
  ]
...
to reproduce-bears  ;; ber procedure
  if random-float 100 < bears-reproduce [  ;; throw "dice" to see if you will reproduce
    set energy (energy / 2)               ;; divide energy between parent and offspring
    hatch 1 [ rt random-float 360 fd 1 ]  ;; hatch an offspring and move it forward 1 step
  ]
end
...
to reproduce-hunters  ;; ber procedure
  if random-float 100 < hunters-reproduce [  ;; throw "dice" to see if you will reproduce
    set energy (energy / 2)               ;; divide energy between parent and offspring
    hatch 1 [ rt random-float 360 fd 1 ]  ;; hatch an offspring and move it forward 1 step
  ]
end
...
to catch-wolves  ;; wolf procedure
  let prey one-of wolves-here                    ;; grab a random sheep
  if prey != nobody                             ;; did we get one?  if so,
    [ ask prey [ die ]                          ;; kill it
      set energy energy + bears-gain-from-food ] ;; get energy from eating
end

to catch-bears  ;; wolf procedure
  let prey one-of bears-here                    ;; grab a random sheep
  if prey != nobody                             ;; did we get one?  if so,
    [ ask prey [ die ]                          ;; kill it
      set energy energy + hunters-gain-from-food ] ;; get energy from eating
end
...
"bears" 1.0 0 -8431303 true "" "plot count bears"
"hunters" 1.0 0 -7500403 true "" "plot count hunhters"
